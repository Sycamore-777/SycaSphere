...
1. 将 `1e-200` 下溢与足以使平方溢出的有限标准差放入同一参数化边界回归。修复前两例
   均为本维护项要求下的 RED：下溢例错误地产生零方差；溢出例虽已在稍后的协方差验证
   中被拒绝，因而已有宽泛的安全拒绝，但未在 `StandardDeviations` 边界以已批准的统一
   可表示性消息失败；
2. 实现最小可表示性检查，使该组合参数化回归的下溢和溢出两例都转为 GREEN，并将两者
   统一收口到标准差验证边界；
3. 不将溢出的后续协方差拒绝误记为本维护项所需边界契约的立即 GREEN；
